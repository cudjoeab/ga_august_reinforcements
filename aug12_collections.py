...
# 1. Add another key-value pair to each article 
# where the key is "views" and the value is 0.

print(dictionary.keys()) 

print('\n')

for entry in range(len(dictionary['response']['results'])):
    dictionary['response']['results'][entry]['views'] = 0 
# print(dictionary['response']['results'])

# 2. Write a method called read_article 
# that randomly selects an article and 
# increases the articles "views" by one each time it's randomly selected.

def read_article(): 
    num_of_articles = len(dictionary['response']['results'])
